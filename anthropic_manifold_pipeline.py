...
author: justinh-rahb, sriparashiva, Seyed Yahya Shirazi
date: 2024-06-20
version: 1.4
license: MIT
...
class Pipeline:
    ...
    def stream_response(self, payload: dict) -> Generator:
        try:
            self._current_response = requests.post(
                self.url, 
            ...
            if self._current_response.status_code == 200:
                self._current_client = sseclient.SSEClient(self._current_response)
                for event in self._current_client.events():
                    try:
                ...
                raise Exception(f"Error: {self._current_response.status_code} - {self._current_response.text}")
        finally:
            self.cleanup_resources()

    def get_completion(self, payload: dict) -> str:
        try:
            self._current_response = requests.post(self.url, headers=self.headers, json=payload)
            if self._current_response.status_code == 200:
                res = self._current_response.json()
